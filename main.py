...
ALIASES = {
    ...
    "lote":"lote", "validade":"validade", "posicao":"posicao", "posição":"posicao",
    "quantidade":"quantidade", "qtd":"quantidade", "status":"status"
}
...
def lookup(rg: str, db: Session = Depends(db), user=Depends(current_user)):
    ...
    return {"found":True, "rg":item.rg, "produto":item.produto, "lote":item.lote,
            "validade":item.validade, "posicao":item.posicao, "quantidade":item.quantidade, "status":item.status}
